...
const saleByDate  = (orders, products) => {
  ...
  orders.map(order=>{
    ...
    order.items.map(item=>{
      sale += item.quantity * products.find(e=> e.id === item.productId).price
      cost += item.quantity * products.find(e=> e.id === item.productId).cost
    ...
    // console.log(date);
    // console.log('sale: ',sale,'cost: ',cost,'quantity: ',quantity);
  })
