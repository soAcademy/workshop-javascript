...
const arr = [1, 2, 4, 5, 3];
let max = -999999, secondMax = -999999;
arr.forEach(num => {
  (num > max) ? [secondMax, max] = [max, num] : (num >= secondMax) && (num!==max) && (secondMax = num);
});
...
const prices = [100, 110, 120, 130, 80, 99, 64, 140, 150];

const ROC = prices.slice(1).reduce((acc, cur, idx) => {
  acc.push((cur - prices[idx]) / prices[idx]);
  return acc;
}, []);
console.log(ROC);
